...
import pandas as pd
import redis
from ..Recall.recallprocess import config_get
...
def debug_process(source_path,process_path):
    ...
    data['movieactor1'] = data['movieid'].map(lambda x: r.hgetall("mf:" + str(x))['movieactor1'] \
        if len(r.hgetall("mf:" + str(x))) >= 1 else None) \
        .map(lambda x: dict_actortoindex[x] if len(x) >= 1 else None)
    data['movieactor2'] = data['movieid'].map(lambda x: r.hgetall("mf:" + str(x))['movieactor2'] \
        if len(r.hgetall("mf:" + str(x))) >= 1 else None) \
        .map(lambda x: dict_actortoindex[x] if len(x) >= 1 else None)

    data['movieactor3'] = data['movieid'].map(lambda x: r.hgetall("mf:" + str(x))['movieactor3'] \
        if len(r.hgetall("mf:" + str(x))) >= 1 else None) \
        .map(lambda x: dict_actortoindex[x] if len(x) >= 1 else None)
    data['movieactor4'] = data['movieid'].map(lambda x: r.hgetall("mf:" + str(x))['movieactor4'] \
        if len(r.hgetall("mf:" + str(x))) >= 1 else None) \
        .map(lambda x: dict_actortoindex[x] if len(x) >= 1 else None)
    data['moviedirecotr1'] = data['movieid'].map(lambda x: r.hgetall("mf:" + str(x))['moviedirecotr1'] \
        if len(r.hgetall("mf:" + str(x))) >= 1 else None) \
        .map(lambda x: dict_directortoindex[x] if len(x) >= 1 else None)
    data.to_csv(process_path, index=False)

def wideanddeep(flag):
    train_source = config_get('train_data_source', 'train_source')
    test_source = config_get('train_data_source', 'test_source')
    all_source = config_get('train_data_source', 'all_source')
    training_samples_file_path = config_get('train_data_source', 'training_samples_file_path')
    test_samples_file_path = config_get('train_data_source', 'test_samples_file_path')
    all_samples_file_path = config_get('train_data_source', 'all_samples_file_path')
    if flag == 'debug':
        debug_process(train_source, training_samples_file_path)
        debug_process(test_source, test_samples_file_path)
        debug_process(all_source, all_samples_file_path)
    elif flag == 'train':
        ...
        recalldatapath = config_get('data_source', 'data_for_model')
        recalldatas = pd.read_csv(recalldatapath)
        predict_dataset = get_predataset(recalldatapath)
        model = tf.keras.models.load_model(config_get('model', 'wideanddeep'))
        ...
        recalldatas.to_csv(config_get('data_source', 'predictions_of_svd'))
