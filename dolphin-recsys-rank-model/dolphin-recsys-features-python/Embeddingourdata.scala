...
object Embeddingourdata {
  ...
  def processItemSequence(sparkSession: SparkSession, rawSampleDataPath: String): RDD[Seq[String]] ={
    ...
    val userSeq = ratingSamples
      .where(col("评分") >= 3.5)
      .groupBy("用户ID")
      .agg(sortUdf(collect_list(struct("movieid", "评论时间"))) as "movieIds")
      .withColumn("movieIdStr", array_join(col("movieIds"), " "))

    userSeq.select("用户ID", "movieIdStr").show(10, truncate = false)
  ...
  def generateUserEmb(sparkSession: SparkSession, rawSampleDataPath: String, word2VecModel: Word2VecModel, embLength:Int, embOutputFilename:String, saveToRedis:Boolean, redisKeyPrefix:String): Unit ={
    ...
    ratingSamples.collect().groupBy(_.getAs[String]("用户ID"))
      .foreach(user => {
        val userId = user._1
    ...
    val embFolderPath = this.getClass.getResource("/webroot/ourmodeldata/")
...
//    val synonyms = model.findSynonyms("燃情迈阿密", 20)
